...
function renderDtrPage(
  ...
  doc
    ...
    .fontSize(9)
    .fillColor("#CBD5E1")
    .text(`Page ${pageIndex + 1} of ${totalPages}`, PAGE_WIDTH - PAGE_MARGIN - 84, 82, {
      width: 84,
      align: "right",
    });
  ...
  drawMetric(doc, PAGE_MARGIN, 182, "EMPLOYEE", employeeName, 224);
  ...
  const projectAccountValue = entry.project_account ?? "-";
  const weekWidth = 156;
  ...
  drawLabelValue(doc, weekX, summaryLabelY, "WEEK RANGE", weekRangeValue, weekWidth);
  drawLabelValue(doc, projectX, summaryLabelY, "PROJECT / ACCOUNT", projectAccountValue, projectWidth);
  ...
  const maxNotesHeight =
    PAGE_HEIGHT - footerReserve - imageBlockHeight - imageBlockGap - notesTextY;
  const notesText = entry.notes?.trim() || "No notes provided.";
  let notesFontSize = 10;
  let notesLineGap = 6;
  let notesTextHeight = measureTextHeight(
    doc,
    notesText,
    CONTENT_WIDTH - 40,
    "Helvetica",
  ...
  while (notesTextHeight > maxNotesHeight && notesFontSize > 8) {
    ...
    notesTextHeight = measureTextHeight(
      doc,
      notesText,
      CONTENT_WIDTH - 40,
      "Helvetica",
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!employeeId || !weekStart) {
      return NextResponse.json({ error: "Missing employee_id or week_start" }, { status: 400 });
    }
    ...
    const { data: entries, error } = await supabase
      .from("dtr_entries")
      .select(
        "id, work_date, week_start, week_end, start_time, end_time, project_account, notes, image_link, duration_minutes, status",
      )
    ...
    if (!entries?.length) {
      return NextResponse.json({ error: "No DTR entries for this week" }, { status: 404 });
    }
    ...
    entries.forEach((entry, index) => {
      renderDtrPage(
        ...
        entry,
        employee.full_name,
        (managerProfile.full_name ?? "").replace(/\s+/g, " ").trim(),
        index,
        entries.length,
      );
    });
    ...
    const managerName = (managerProfile.full_name ?? "").replace(/\s+/g, " ").trim();
    ...
    const employeeName = employee.full_name?.trim() || "employee";
    const filename =
      `ph-seo-parttimer-weekly-dtr-${slugifyFilenamePart(employeeName)}-` +
      `${formatFileDate(entries[0].week_start)}-to-${formatFileDate(entries[0].week_end)}.pdf`;
    normalizedPdf.setTitle(filename.replace(/\.pdf$/i, ""));
    normalizedPdf.setSubject("PH SEO Parttimer weekly DTR export");
    ...
    console.error("weekly dtr export failed", error);
